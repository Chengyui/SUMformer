# -*- coding: utf-8 -*-
"""
Created on Sun May 21 16:00:47 2023
...
class MidMetaNet(nn.Module):
    ...
    def __init__(self, channel_in, channel_hid,channel_out, N2,
        ...
        # upsample
        enc_layers.append(MetaBlock(
            channel_hid, channel_out, input_resolution,
            mlp_ratio, drop, drop_path=drop_path, layer_i=self.N2-1))
    ...
    def forward(self, x):
        ...
        y = z.reshape(B, self.out_len, C, H, W)
        return y
...
class SimVP_Model(nn.Module):
    ...
    def __init__(self, in_shape, hid_S=16, hid_T=256, N_S=4, N_T=4,
        ...
        self.hid = MidMetaNet(T*hid_S, hid_T,N_T*hid_S, N_T,
    ...
    def forward(self, x_raw, **kwargs):
        ...
        hid = self.hid(z)
        hid = hid.reshape(B*self.N_T, C_, H_, W_)
        if hid.shape[0]==skip.shape[0]:
            Y = self.dec(hid, skip)
        else:
            Y = self.dec(hid)
        Y = Y.reshape(B, self.N_T, C, H, W)
