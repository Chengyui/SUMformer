...
from .sum_encoder_Unet import Encoder
...
class Sumformer(nn.Module):
    def __init__(self, data_dim, in_len, out_len, seg_len, win_size=4,
                 factor=10, d_model=512, d_ff=None, n_heads=8, e_layers=3,
        ...
        # Encoder
        self.encoder = Encoder(e_layers, win_size, d_model, n_heads, d_ff, block_depth=3, \
                               dropout=dropout, in_seg_num=(self.pad_in_len // seg_len), factor=factor)

        self.predict_linear = nn.ModuleList(
            [nn.Linear(win_size**(e_layers-1)*d_model, self.out_len) for a in range(e_layers)])
    ...
    def forward(self, x_seq, mode="train", frozen=False):
        ...
        if mode == "pretrain":
            mask_predict = rearrange(predict_y, 'b (seg_num seg_len) out_d-> b seg_num out_d seg_len',
    ...
    def density_sampling(self, x_seq, mode="softmax"):
        ...
        if mode == "uniform":
            ...
            sample_index = torch.multinomial(density / density.sum(dim=-1), int(0.4 * density.shape[-1]),
                                             replacement=False)
        elif mode == "softmax":
