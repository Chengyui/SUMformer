...
from .sum_embed import DSW_embedding,DSW_embedding_vit
...
class Sumformer(nn.Module):
    def __init__(self, data_dim, in_len, out_len, seg_len, win_size = 4,
                ...
                dropout=0.0, baseline = False, device=torch.device('cuda:0'),layer_scaler=1,spatial_patchsize=2):
        # modified spatial_patchsize, we set 2,4,8 in our paper.
        super(Sumformer, self).__init__()
        self.data_dim = data_dim//(spatial_patchsize*spatial_patchsize*2)
        self.in_len = in_len
        self.out_len = out_len
        self.seg_len = seg_len
        self.merge_win = win_size
        self.d_layers = 0
        self.spatial_patchsize = spatial_patchsize
        ...
        # Embedding
        self.enc_value_embedding = DSW_embedding_vit(seg_len, d_model,spatial_patchsize)
        ...
        self.predict_linear = nn.ModuleList([nn.Linear(ceil(in_len / (seg_len * win_size ** (a)))*d_model,self.out_len*(2*spatial_patchsize*spatial_patchsize)) for a in range(e_layers)])
    ...
    def forward(self, x_seq,mode="train",frozen=False):
        ...
        x_seq = rearrange(x_seq,'b t c (h p1) (w p2) -> b t (h w) (c p1 p2)',p1=self.spatial_patchsize,p2=self.spatial_patchsize)
        if (self.baseline):
        ...
        predict_y = rearrange(predict_y,'b (t c p1 p2) (h w) -> b t c (h p1) (w p2)',c=C,p1=self.spatial_patchsize,p2=self.spatial_patchsize,h=H//self.spatial_patchsize)
        if mode=="pretrain":
