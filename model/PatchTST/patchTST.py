...
from ..PatchTST.layers.pos_encoding import *
from ..PatchTST.layers.basics import *
from ..PatchTST.layers.attention import *
...
class PatchTST(nn.Module):
    ...
    def __init__(self, c_in:int, target_dim:int, patch_len:int, num_patch:int,
        ...
        self.pretrain_head = PretrainHead(d_model, patch_len, head_dropout) # custom head passed as a partial func with all its kwargs
        self.prediction_head = PredictionHead(individual, self.n_vars, d_model, num_patch, target_dim, head_dropout)


    def forward(self, z,mode="train",frozen=False):
        """
        [B,T,C,H,W]
        ...
        z = rearrange(z,'b (num_patch patch_len) n_vars -> b num_patch n_vars patch_len',patch_len=self.patch_len)
        if mode=="pretrain":
            masked_z,_,mask,_ = self.random_masking(z,0.4)
        else:
            masked_z = z
        masked_z = self.backbone(masked_z)                                                                # z: [bs x nvars x d_model x num_patch]
        if mode=="pretrain":
            masked_z = self.pretrain_head(masked_z)
        else:
            masked_z = self.prediction_head(masked_z)
        # z: [bs x target_dim x nvars] for prediction
        #    [bs x target_dim] for regression
        #    [bs x target_dim] for classification
        #    [bs x num_patch x n_vars x patch_len] for pretrain
        if mode=="pretrain":
            loss = self._loss(masked_z,z,mask)
            return loss
        masked_z = rearrange(masked_z,'b t (c h w) -> b t c h w',c = C,h=H,w=W)
        return masked_z
...
class PatchTSTEncoder(nn.Module):
    def __init__(self, c_in, num_patch, patch_len, 
                 ...
                 d_ff=256, norm='LayerNorm', attn_dropout=0., dropout=0., act="gelu", store_attn=False,
