...
if __name__ == "__main__":
    ...
    model = SwinLSTM(img_size=64, patch_size=(2,2),
                     in_chans=1, embed_dim=128,
                     depths_downsample=[2,6], depths_upsample=[6,2],
                     num_heads=[4,8], window_size=4).to("cuda:0")

    inputs = torch.randn((16,10,1,64,64)).to("cuda:0")
    output = model(inputs[:,0],[None,None],[None,None])
