...
class Crossformer(nn.Module):
    def __init__(self, data_dim, in_len, out_len, seg_len, win_size = 4,
                factor=10, d_model=64, d_ff = 512, n_heads=8, e_layers=3,d_layers=3+1,
        ...
        self.decoder = Decoder(seg_len, d_layers, d_model, n_heads, d_ff, dropout, \
                                    out_seg_num = (self.pad_out_len // seg_len), factor = factor)
        self.predict_linear = nn.Linear(d_model,seg_len*win_size**(e_layers-1))
    ...
    def forward(self, x_seq):
        ...
        predict_y = self.decoder(dec_in, enc_out[-self.d_layers:])
